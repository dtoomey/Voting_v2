...
namespace VotingService
{
    ...
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        public void ServiceRequestStop(string requestTypeName, string activityId)
        {
            WriteEvent(ServiceRequestStopEventId, requestTypeName, activityId);
        }

        private const int ServiceRequestFailedEventId = 7;
        [Event(ServiceRequestFailedEventId, Level = EventLevel.Error, Message = "Service request '{0}' failed. Activity ID: {1}", Keywords = Keywords.Requests)]
        public void ServiceRequestFailed(string requestTypeName, string activityId, string exception)
        {
            WriteEvent(ServiceRequestFailedEventId, exception, activityId);
        }
